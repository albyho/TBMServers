...
*/
function CommandBuffer (head_length, getBodyLength) {
...
util.inherits(CommandBuffer, events.EventEmitter);

/**
* @description add buffer to read, and check for enough data
* @param {Buffer} buff
*/
CommandBuffer.prototype.addBuffer = function (buff) {
...
*/
CommandBuffer.prototype._checkEnoughData = function () {
    ...
    if (!this.head) {
        ...
        if (this.curr_pos + this.bytes_to_read <= first_buff.length) {
            ...
            if (this.curr_pos >= this.head_length &&
                ...
                if(this.total_unread_length <= 0) {
                    this.emit('packet', packet);
                } else {
                    var data = this.buffers[0].slice(this.curr_pos, this.curr_pos + this.total_unread_length);
                    this.emit('packet', packet, data);
                }
            }
        } else {
            this.head = Buffer.allocUnsafe(this.head_length);
        ...
        var buff = buff = Buffer.allocUnsafe(this.head_length + this.bytes_to_read);
        this.head.copy(buff);
...
*/
CommandBuffer.prototype._skipData = function (length_to_skip) {
...
*/
CommandBuffer.prototype._dataToBuffer = function (buff, start_pos) {
...
// 基于: https://github.com/freedaxin/head_body_buffers/blob/master/head_body_buffers.js